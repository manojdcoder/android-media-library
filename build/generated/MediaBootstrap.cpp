...
#define TAG "com.mykingdom.media"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Media_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Media.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::MediaBindings::lookupGeneratedInit(
...
static void Media_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Media_getBinding)->GetFunction());
}

static void Media_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::MediaBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry MediaBinding = {
	"com.mykingdom.media",
	Media_init,
	Media_dispose
};
...
Java_com_mykingdom_media_MediaBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.mykingdom.media", &MediaBinding);
	titanium::KrollBindings::addExternalLookup(&(::MediaBindings::lookupGeneratedInit));
}
